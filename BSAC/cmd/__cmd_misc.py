...
def _find_wpe_parallel_TEST( hpar , hcov , *csp , n_samples = 1 , pwpe = None , side = None , clim = None , perwpe = None , sample_idx = None , event = None ):##{{{
	...
	## And save in the zarr file
	sel = (slice(None),) + sample_idx + (slice(None),) + csp
	event.set_orthogonal_selection( sel , output.transpose(2,0,1) )
...
## run_bsac_cmd_misc_wpe_TEST ##{{{
@log_start_end(logger)
def run_bsac_cmd_misc_wpe_TEST():
	...
	while any(nfind):
		i = np.argmin(nsizes)
		while mem_use > bsacParams.total_memory or np.prod(blocks) > 50 * bsacParams.n_workers * bsacParams.threads_per_worker:
			mem_use = fmem_use(blocks)
			if blocks[i] < 2:
				blocks[i] = 1
				break
	...
	for idx in itt.product(*[range(0,s,b) for s,b in zip(clim.s_spatial + (n_samples,),blocks)]):
		...
		## Coordinates
		csp = []
		for i,d in enumerate(d_spatial):
			c = c_spatial[d][spatial_idx[i]].astype(int)
			c[:] = np.arange(idx[i],idx[i]+blocks[i],1)
			csp.append(c.chunk( {d : 1 for d in c.dims} ))
		...
		## Find event
		xr.apply_ufunc( _find_wpe_parallel , shpar , shcov , *csp,
		                    input_core_dims  = [["hpar"],["hpar0","hpar1"]] + [[] for _ in range(len(csp))],
		                    output_core_dims = [[]],
		                    output_dtypes    = [],
		                    vectorize        = True,
		                    dask             = "parallelized",
		                    kwargs           = { "n_samples" : len(ssamples) , "pwpe" : pwpe , "side" : side , "clim" : clim , "perwpe" : perwpe , "sample_idx" : sample_idx , "event" : event },
		                    ).compute()
