...
def _find_wpe_parallel( hpar , hcov , n_samples = 1 , p = None , side = None , clim = None , perwpe = None ):##{{{
	...
	## Init output
	output = np.zeros( (n_samples,npers) ) + np.nan
	...
	## Init eventL and eventH
	eventL = nslaw.icdf_sf( min(     1e-6 ,         p  / 100 ) , side , **dpars )
	eventH = nslaw.icdf_sf( max( 1 - 1e-6 ,1 - (1 - p) / 100 ) , side , **dpars )
	if side == "right":
		eventL,eventH = eventH,eventL
	eventL[:] = np.nanmin( eventL , axis = 0 )
	eventH[:] = np.nanmax( eventH , axis = 0 )
	...
	while res > eps:
		...
		## Probability of the central event
		pM = 1 - np.prod( 1 - nslaw.cdf_sf( eventM , side , **dpars ) , axis = 0 )
...
#		for idx in itt.product(*[range(0,s,block) for s in (n_samples,) + clim.s_spatial]):
		for idx in itt.product(*[range(0,s,block) for s in clim.s_spatial]):
			...
			## Indexes
#			sample_idx  = (slice(idx[0],idx[0]+block,1),)
#			spatial_idx = tuple([slice(s,s+block,1) for s in idx[1:]])
			spatial_idx = tuple([slice(s,s+block,1) for s in idx])
...
#			ssamples = samples[sample_idx[0]]
			...
			xevent = xr.apply_ufunc( _find_wpe_parallel , shpar , shcov ,
			                    ...
			                    output_core_dims = [["sample","period"],],
			                    output_dtypes    = [shpar.dtype],
			                    vectorize        = True,
			                    dask             = "parallelized",
			                    kwargs           = { "n_samples" : len(ssamples) , "p" : p , "side" : side , "clim" : clim , "perwpe" : perwpe },
			                    dask_gufunc_kwargs = { "output_sizes" : { "sample" : len(ssamples) , "period" : len(periods) } }
			                    ).assign_coords( { "sample" : ssamples , "period" : periods } ).transpose( *(("sample","period") + d_spatial) ).compute()
