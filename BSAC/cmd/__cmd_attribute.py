...
def _attribute_fintensity_parallel( *args , ovars , nslaw_class , side , mode , ci ):##{{{
...
def run_bsac_cmd_attribute_fintensity(arg):
	...
	for i,d in enumerate(cdims):
		if d in [mode] + list(zdraw[ovars[0]].dims)[1:]:
	...
	## Output
	dims   = cdims   + [mode   ] + list(zdraw[ovars[0]].dims)[1:]
	coords = ccoords + [modes  ] + list(zdraw[ovars[0]].coords)[1:]
	shape  = cshape  + [n_modes] + list(zdraw[ovars[0]].shape)[1:]
	out = {
	    "pF" : XZarr.from_value( np.nan , shape = shape , dims = dims , coords = coords , zfile = random_zfile( os.path.join( tmp , "pF" ) ) ),
	    "pC" : XZarr.from_value( np.nan , shape = shape , dims = dims , coords = coords , zfile = random_zfile( os.path.join( tmp , "pC" ) ) ),
	    "RF" : XZarr.from_value( np.nan , shape = shape , dims = dims , coords = coords , zfile = random_zfile( os.path.join( tmp , "RF" ) ) ),
	...
	for idx in itt.product(*[range(0,s,block) for s in clim.s_spatial]):
		...
		#
		res = xr.apply_ufunc( _attribute_fcintensity_parallel , *[draw[ovar] for ovar in ovars] , bias , IF ,
		                    input_core_dims    = [[ "sample","time","period"] for _ in range(len(draw))] + [[]] + [[]],
		                    output_core_dims   = [[mode,"time","period"] for _ in range(8)],
		                    output_dtypes      = [draw[ovars[0]].dtype for _ in range(8)],
		                    vectorize          = True,
		                    dask               = "parallelized",
		                    kwargs             = { "ovars" : ovars , "nslaw_class" : clim._nslaw_class , "side" : side , "mode" : mode , "ci" : ci },
		                    dask_gufunc_kwargs = dask_gufunc_kwargs
		                    )
